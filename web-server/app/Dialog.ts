...
export default class Dialog extends PIXI.Container {
    ...
    constructor(title: string | PIXI.Sprite | PIXI.Texture, content: PIXI.Container, ani_control: TWEEN, options?: any) {
        super();
    ...
    close() {
        ...
        var cur_y = this.y;
        this.ani.Tween(this)
            .to({ x: this.x + this.width / 2, y: this.y + this.height / 2 }, B_ANI_TIME)
            .easing(TWEEN.Easing.Quintic.In)
            .start()
            .onComplete(() => {
